<template>
  <div class="trx">
    <div class="tx-link">
    ...
    </div><span class="operation break-word">
      <div v-if="typeOp == 'curation_reward'">
        <a :href="'#/@'+op.curator">{{op.curator}}</a> curation reward: {{this.vests2sp(op.reward)}} for <a
          :href="link(op.comment_author,op.comment_permlink)">{{linkCut(op.comment_author,op.comment_permlink)}}</a>
      </div>
      <div v-else-if="typeOp == 'vote'">
        <a :href="'#/@'+op.voter">{{op.voter}}</a> upvote <a
          :href="link(op.author,op.permlink)">{{linkCut(op.author, op.permlink)}}</a> ({{(op.weight/100).toFixed(2)}}%)
      </div>
      <div v-else-if="typeOp == 'transfer'">
        <a :href="'#/@'+op.from">{{op.from}}</a> transfer {{op.amount}} to <a :href="'#/@'+op.to">{{op.to}}</a>. Memo:
        <span class="memo">{{op.memo}}</span>
      </div>
      <div v-else-if="typeOp == 'delegate_vesting_shares'">
        <a :href="'#/@'+op.delegator">{{op.delegator}}</a> delegate <a :href="'#/@'+op.delegatee">{{op.delegatee}}</a>
        {{this.vests2sp(op.vesting_shares)}}
      </div>
      <div v-else-if="typeOp == 'comment' && op.parent_author != ''">
        <a :href="'#/@'+op.author">{{op.author}}</a> replied to <a
          :href="link(op.parent_author,op.parent_permlink)">{{linkCut(op.parent_author,op.parent_permlink)}}</a>. <span
          class="memo">{{op.body.substring(0,140)}}{{op.body.length>140?'...':''}}</span>
      </div>
      <div v-else-if="typeOp == 'comment' && op.parent_author == ''">
        <a :href="'#/@'+op.author">{{op.author}}</a> authored a post: <a
          :href="link(op.author,op.permlink)">{{linkCut(op.author,op.permlink)}}</a>
      </div>
      ...
      <div v-else-if="typeOp == 'create_claimed_account'">
        <a :href="'#/@'+op.creator">{{op.creator}}</a> create claimed account <a
          :href="'#/@'+op.new_account_name">{{op.new_account_name}}</a>
      </div>
      <div v-else-if="typeOp == 'account_create_with_delegation'">
        <a :href="'#/@'+op.creator">{{op.creator}}</a> create account <a
          :href="'#/@'+op.new_account_name">{{op.new_account_name}}</a>. Fee: {{op.fee}}. Delegation:
        {{this.vests2sp(op.delegation)}}
      </div>
      <div v-else-if="typeOp == 'account_create'">
        <a :href="'#/@'+op.creator">{{op.creator}}</a> create account <a
          :href="'#/@'+op.new_account_name">{{op.new_account_name}}</a>. Fee: {{op.fee}}.
      </div>
      ...
      <div v-else-if="typeOp == 'account_witness_vote'">
        <a :href="'#/@'+op.account">{{op.account}}</a> <span v-if="op.approve">approve</span><span
          v-else>unapprove</span> witness <a :href="'#/@'+op.witness">{{op.witness}}</a>
      </div>
      <div v-else-if="typeOp == 'claim_reward_balance'">
        <a :href="'#/@'+op.account">{{op.account}}</a> claim reward: {{op.reward_sbd}}, {{op.reward_steem}},
        {{this.vests2sp(op.reward_vests)}}
      </div>
      <div v-else-if="typeOp == 'comment_benefactor_reward'">
        <a :href="'#/@'+op.benefactor">{{op.benefactor}}</a> benefactor reward: {{op.sbd_payout}}, {{op.steem_payout}},
        {{this.vests2sp(op.vesting_payout)}} for <a
          :href="link(op.author,op.permlink)">{{linkCut(op.author,op.permlink)}}</a>
      </div>
      <div v-else-if="typeOp == 'author_reward'">
        <a :href="'#/@'+op.author">{{op.author}}</a> author reward: {{op.sbd_payout}}, {{op.steem_payout}},
        {{this.vests2sp(op.vesting_payout)}} for <a
          :href="link(op.author,op.permlink)">{{linkCut(op.author,op.permlink)}}</a>
      </div>
      ...
      <div v-else-if="typeOp == 'custom_json' && op.id == 'follow' && op.json[0] == 'reblog'">
        <a :href="'#/@'+op.json[1].account">{{op.json[1].account}}</a> reblog <a
          :href="link(op.json[1].author,op.json[1].permlink)">{{linkCut(op.json[1].author, op.json[1].permlink)}}</a>
      </div>
